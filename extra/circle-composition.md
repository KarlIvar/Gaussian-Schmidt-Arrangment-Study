...
Suppose the circles are handed to us as \(X(\hat{\mathbb{R}})\), \(Y(\hat{\mathbb{R}})\) with \(X, Y \in \mathrm{SL}_2(\mathbb{Z}[i])\) known. Three facts, all machine-verified in [extra/scripts/matrix_composition_check.py](scripts/matrix_composition_check.py):
...
[extra/scripts/composition_check.py](scripts/composition_check.py) checks, for every ordered pair of primitive classes at every odd \(n \le 41\) (1913 pairs):
...
The figure above ([extra/scripts/make_composition_figure.py](scripts/make_composition_figure.py)) shows both recipes at level \(n = 9\).

[extra/scripts/matrix_composition_check.py](scripts/matrix_composition_check.py) verifies §5: the \(W_X = \tfrac i2(Z_X + nI)\) read-off (93 matrices, including slid representatives), the aligned diag-conjugation composition (245 ordered pairs of classes, both conjugates equal as matrices and reducing to the Gauss composite), the failure of naive products to be class-well-defined, and the trace-pairing identity.
