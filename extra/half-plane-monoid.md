...
— the elements of the Bianchi group that push the upper half plane *into itself*. Everything below is proved and machine-verified in exact arithmetic ([extra/scripts/omega.py](scripts/omega.py), [extra/scripts/omega_verify.py](scripts/omega_verify.py): 41/41 checks pass).
...
*Proof sketch (verified in [extra/scripts/omega_verify.py](scripts/omega_verify.py), §4 of the output).*
...
### 9.1 The atom through a point ([extra/scripts/omega.py](scripts/omega.py), `atom_through`)
...
### 9.3 Nested disks around \(z\) ([extra/scripts/apollonian_chain_demo.py](scripts/apollonian_chain_demo.py))
...
[extra/scripts/omega_verify.py](scripts/omega_verify.py) — 41 checks, all passing:
