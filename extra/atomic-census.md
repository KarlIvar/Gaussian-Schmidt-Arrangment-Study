...
Everything with finite content is machine-verified in exact arithmetic: [extra/scripts/atomic_census.py](scripts/atomic_census.py), **4414 checks, all passing** (all odd levels \(n \le 41\); random product tests; the density experiment to \(X = 1601\)).
...
[extra/scripts/atomic_census.py](scripts/atomic_census.py) — **4414 checks, all passing** (`python3 extra/scripts/atomic_census.py 41`; density mode `asym`):
...
- [extra/scripts/atomic_census.py](scripts/atomic_census.py) — the full census and all checks of §7. `python3 extra/scripts/atomic_census.py 41`; `python3 extra/scripts/atomic_census.py asym 1601` for §6.
- Depends on [extra/scripts/omega.py](scripts/omega.py) (monoid, factorisation, exact disk arithmetic).
