...
**Update (2026-08-28, spectral-geometry session):** the folder [spectral-geometry/](spectral-geometry/) now exists and opens the spectral front: the plane above the real line is distinguished as an immersed modular surface \(Y \subset \mathrm{PSL}_2(\mathbb{Z}[i])\backslash\mathbb{H}^3\); \(\alpha\) is identified as \(\cosh\) of the ortho-distance to it with the CM points as the feet of the perpendiculars; the ortho-length spectrum \(\{\operatorname{arccosh} n\}\) carries multiplicities \(H(n^2-1)\) ([spectral-geometry/orthogeodesics.md](spectral-geometry/orthogeodesics.md)); and the Catalan constant of the Euclidean count is proved to be the volume ratio \(\mathrm{area}(Y)/\mathrm{vol}(M)\) via Eisenstein series, scattering (\(\varphi(s) = \frac{\pi}{s}\zeta_K(s)/\zeta_K(s+1)\)) and an exact Mellin/period identity — closing Remark 1 of [euclidean-counting.md](euclidean-counting.md) ([spectral-geometry/eisenstein-catalan.md](spectral-geometry/eisenstein-catalan.md)). This advances 3.1 substantially (the geodesic-multiplicity statement is now a theorem; the trace-formula run remains) and gives 3.2 its Kudla–Millson frame.
