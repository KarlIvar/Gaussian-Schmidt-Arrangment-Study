...
$\langle M, N\rangle = \tfrac12 \tr(M N^{\mathrm{adj}})$ with
$N^{\mathrm{adj}}$ the adjugate, and
...
-z_0/r \\ -\bar z_0/r & (|z_0|^2 - r^2)/r \end{smallmatrix}\bigr)$, and
for $M = (A_1, B_1, C_1)$, $N = (A_2, B_2, C_2)$ one computes
$\tfrac12\tr(MN^{\mathrm{adj}}) = \tfrac12(A_1C_2 + C_1A_2) -
\re(B_1\bar B_2)$, which for two proper circles is the classical
inversive product $\frac{|z_1 - z_2|^2 - r_1^2 - r_2^2}{2r_1r_2}$ and
for $N = M_0$ evaluates to $y_0/r$.  (3) is proved in
...
Hilbert 90 trivializes it; over $\Zi$ it is a genus question).  For
the classical theory of binary Hermitian forms over imaginary
quadratic rings, going back to Bianchi, see \cite[Ch.~9]{EGM1998}.
...
At level $n$, $m_1$ is the CM point of the class $[f]$, $\bar m_2 \in
\HH$ is the CM point of the class $\hat\sigma[f] = [\idealr_n][f]^{-1}$,
and $-m_2 \in \HH$ is the CM point of the class $[\idealr_nf]$;
consequently $\beta_1 = j(\tau_{[f]})$ and $\beta_2 =
j(\tau_{[\idealr_n][f]^{-1}})$, with $\bar\beta_2 = j(-m_2) =
j(\tau_{[\idealr_nf]})$, are singular moduli of discriminant
$D = 1 - n^2$, and over the primitive level-$n$ classes
\[
...
Theorem \ref{thm:classformula} for $\bar m_2$; then $-m_2$, the mirror
image of $\bar m_2$, is the point of the inverse class
$[\idealr_nf]$, and $j$ of mirror points are complex conjugates.  The
product formula is the definition of the ring class polynomial, whose
integrality is classical \cite[\S13]{Cox2013}.
\end{proof}
...
Write $j_i := j(\idealb_i)$, so $j_1 = \beta_1$ and $j_2 =
\bar\beta_2$ is the singular modulus of the class $[\idealr_nf]$
(Proposition \ref{prop:singularmoduli}), and write the sixth power of
the phase in the closed form
\[
  u_f^6 \;=\; \varepsilon^6\,\mu^{-12}\,
  \frac{j_1^4(j_1 - 1728)^3}{j_2^4(j_2 - 1728)^3}
...
$j_1^4(j_1 - 1728)^3$ are algebraic integers; and the prime
support of the algebraic integer $j_2^4(j_2 - 1728)^3$ lies in
...
$\Cl(\mathcal{O}_n)$ for $n \ge 2$, and $\mathrm{Tr}^E_1 := 492$; then
$\sum_{g \mid n}\mathrm{Tr}^E_{n/g} = t(4n^2)$, Zagier's trace
\cite{Zagier2002}; verified for all
