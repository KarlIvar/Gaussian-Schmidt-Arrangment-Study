...
$$
Certified at 140–600 digits with the absolute-error criterion (experiment D; every coefficient carries 100+ spare digits; \(P^{(6)}\) is then derived *exactly* from \(P^{(2)}\) by the cube resultant and re-checked numerically):
...
> **Observed law (certified at all listed levels).**
> $$
> M(n) \;=\; \pm \prod_{\substack{p^k \| n \\ p \text{ not split}}} p^{\;\frac{6}{e_p}\cdot\frac{p^k - 1}{p - 1}\; N_e(n/p^k)},
> $$
> with \(e_2 = 2\) (ramified), \(e_p = 1\) (inert); **split primes contribute nothing at all** — e.g. \(M(5) = M(13) = M(25) = 1\) exactly. The sign is \(-1\) exactly at \(n = 4, 8, 16\) (the levels \(2^k\), \(k \ge 2\), in the computed range).

The dichotomy is Deuring reduction: split \(p\) = ordinary reduction of the lemniscatic curve (the \(p\)-isogeny volcano is horizontal at the top and the valuations cancel over the class group), non-split \(p\) = supersingular reduction, where the geometric series \(\tfrac{p^k-1}{p-1} = 1 + p + \cdots + p^{k-1}\) is exactly the growth of quasi-canonical-lifting valuations (Gross). Proving the law — presumably by the Gross–Keating/quasi-canonical calculus for the pair \((\mathbb{Z}[i], \mathcal{O}_n)\) — is a concrete target (§6).

### 5.6 What changed relative to the hyperbolic phase
...
**6.1 Exact valuations of \(\lambda_n\) and of the mass.** Two concrete local problems at \(p \mid n\): (a) the per-coefficient denominators of \(e_k(\{v^2\})\) (data: \(7^3, 7^6, 7^9, 7^{13}\) at \(n=7\); \(3^2, 3^6, 3^{16}, 3^{19}, \ldots\) at \(n = 9\)) — a Newton-polygon-over-classes statement; (b) the mass law of §5.5, which the quasi-canonical valuation formulas of Gross (and Gross–Keating) should prove outright, including the special role of \(p = 3\) (where \(6\frac{p^k-1}{p-1} = 3(p^k-1)\) coincidentally agrees with the \(p = 2\) shape) and the sign at \(2^k\). First move: compute \(v_\mathfrak{p}(\Delta(\Lambda_\mathfrak{c})/\Delta(\mathbb{Z}[i]))\) class-by-class \(\mathfrak{p}\)-adically for \(n = 7, 9\) and match against the quasi-canonical filtration level of \(\Lambda_\mathfrak{c} \otimes \mathbb{Z}_p\).
...
**6.12 Heights: the mass as an arithmetic intersection number.** \(\log M(n)\) is a Faltings-height difference between the conductor-\(n\) locus and the lemniscatic point, and §5.5's law is exactly the shape of an intersection with the supersingular locus (split primes: zero; non-split: geometric series). Prove the law by Gross–Keating/quasi-canonical machinery and, in the other direction, interpret \(H_{-4n^2}(1728)\) (the same-field collision) via degenerate Gross–Zagier — the arithmetic self-intersection of the family with its own \(n = 1\) stratum. Together with 6.6 this would give the Euclidean phase a complete archimedean-plus-finite height story.
...
- Precisions: structure exact (integer arithmetic); analytic experiments at 120–600 digits per level (table `DPS` in the script).
