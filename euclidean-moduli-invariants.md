...
Contrast with the hyperbolic Theorem C (moduli-invariants.md §5.9): same shape — *one level, one integer polynomial* — but the Euclidean polynomial is **monic**: the hyperbolic denominators (Gross–Zagier primes of \((1-n^2, -3), (1-n^2, -4)\)) came from kernel values at *both* CM points in a ratio; here there is one kernel value and its elliptic-point factors \(\beta^4(\beta-1728)^3\) sit in the *numerator*. All denominators of the Euclidean phase live over \(n\) (§5.5), and \(n^2\) clears them. (§5.6 sharpens all of this from sixth powers to squares, for every \(n\), and adds irreducibility.)
...
The descent \(u^6 \to u^2\) at first looked like it needed the Weber function: \(u^2 = -\beta\,(\beta-1728)\,\gamma_2(w)\,R_4(\Lambda)\) with \(\gamma_2 = j^{1/3}\) (in the ring class field only when \(3 \nmid \operatorname{disc}\)) and \(R_4(\Lambda) = \eta(w)^8/(c^4\,\eta(i)^8)\), yet the certified integrality held also at \(3 \mid n\). §5.6 resolves this: \(\gamma_2(w)R_4(\Lambda) = 12\,g_2(\Lambda)/g_2(\mathbb{Z}[i])\) — the product is a root-free weight-balanced \(g_2\)-ratio, no cube root was ever extracted, and the square-level rationality and irreducibility become theorems for **every** \(n\) (Theorem 5). Only the further descent to first powers (the disk polynomial \(P^{(2)}_n(x^2)\)) remains a computed-level statement.
...
### 5.6 Irreducibility: one level, one Galois orbit — a theorem for every level

The key that turns the certified irreducibility of §5.4 into a theorem for **all** \(n\) is a root-free closed form for the square-phase. From \(j' = -2\pi i\,E_4^2E_6/\Delta\) and \(E_4^3 = j\Delta\), \(E_6^2 = (j - 1728)\Delta\),
$$
j'(\tau)^2 \;=\; -(2\pi)^2\, j(\tau)\,\bigl(j(\tau) - 1728\bigr)\,E_4(\tau),
$$
and since \(\Omega^2 = (2\pi)^2\eta(i)^8 = (2\pi)^2 E_4(i)/12\) (because \(\gamma_2(i) = 12\)),
$$
\boxed{\;u_\mathfrak{c}^2 \;=\; -\,12\;\beta_\mathfrak{c}\,\bigl(\beta_\mathfrak{c} - 1728\bigr)\;\frac{g_2(\Lambda_\mathfrak{c})}{g_2(\mathbb{Z}[i])}\;}
$$
— a ring class integer times a **weight-balanced ratio of Weierstrass \(g_2\)-values at the pair \(\Lambda_\mathfrak{c} \subset \mathbb{Z}[i]\)**, with *no root of any modular form extracted anywhere*. (Verified at every class of every computed level, including all \(3 \mid n\); the anchor at \(n = 2\) is the classical \(E_4(2i)/E_4(i) = \tfrac{11}{16}\), which reproduces \(u^2 = -2^43^{10}7^211^4\) on the nose. This also dissolves the §5.4 puzzle: the \(u^6 \to u^2\) descent never needed \(\gamma_2\), so nothing special happens at \(3 \mid n\).) Two lemmas now do all the work.

**Lemma T (transport of balanced pair-values).** Let \(F(\Lambda; L)\) be a rational expression in \(g_2, g_3\) of the two lattices, isobaric of total weight \(0\) under joint scaling \((\Lambda, L) \mapsto (\mu\Lambda, \mu L)\). For \(\sigma \in \mathrm{Aut}(\mathbb{C}/K)\) with idele \(s\) (\([s, K] = \sigma|_{K^{\mathrm{ab}}}\)) and any pair of \(K\)-lattices \(\Lambda \subseteq L\) of index \(n\):
$$
\sigma\bigl(F(\Lambda;\, L)\bigr) \;=\; F\bigl(s^{-1}\Lambda;\, s^{-1}L\bigr),
$$
the adelic action \(s^{-1}(\cdot)\) preserving index, multiplier orders, and (up to the class of \(s\)) proper-ideal classes.

*Proof.* Let \(E = E_\Lambda,\ E' = E_L\) be the Weierstrass curves (a Weierstrass equation determines its period lattice exactly), \(\phi: E \to E'\) the isogeny that is analytically \(z \mapsto z\); it satisfies \(\phi^*\omega' = \omega\) for the Weierstrass differentials. By the main theorem of complex multiplication (Shimura, *Introduction to the Arithmetic Theory of Automorphic Functions*, Thm. 5.4 — stated for arbitrary lattices \(\Lambda \subset K\)), \(E^\sigma\) is analytically \(\mathbb{C}/s^{-1}\Lambda\) via a parametrization \(\xi^\sigma\) compatible with \(\sigma\) on torsion, and likewise \(E'^\sigma\); compatibility on the dense torsion forces \(\phi^\sigma\) to be analytically \(z \mapsto z\) with respect to these parametrizations. Writing \(\Lambda_1, L_1\) for the exact period lattices of the \(\sigma\)-transformed Weierstrass equations, we have \(\Lambda_1 = \theta_E\, s^{-1}\Lambda\), \(L_1 = \theta_{E'}\,s^{-1}L\), and \(\phi^\sigma\) is multiplication by \(\theta_{E'}\theta_E^{-1}\); but \(\sigma\) commutes with algebraic pullback of the algebraically-defined \(\omega\), so \((\phi^\sigma)^*\omega_{L_1} = \omega_{\Lambda_1}\), i.e. \(\theta_{E'} = \theta_E\): the transformed pair is **jointly** homothetic to \((s^{-1}\Lambda,\, s^{-1}L)\). Since \(\sigma(F(\Lambda; L))\) is \(F\) of the transformed pair and \(F\) is jointly scaling-invariant, the claim follows. \(\square\)

**Translation law (squares).** *For \(\sigma \in \mathrm{Gal}(\bar{\mathbb{Q}}/K)\) with Artin class \(\mathfrak{c}(\sigma) \in \mathrm{Cl}(\mathcal{O}_n) \cong \mathrm{Gal}(H_n/K)\):*
$$
\sigma\bigl(u^2_\mathfrak{c}\bigr) \;=\; u^2_{\,\mathfrak{c}(\sigma)^{-1}\mathfrak{c}}\;;
$$
*in particular \(u^2_\mathfrak{c}\) lies in the ring class field \(H_n\), and together with the mirror law (\(\overline{u^2_\mathfrak{c}} = u^2_{\mathfrak{c}^{-1}}\), §5.2) the full group \(\mathrm{Gal}(\bar{\mathbb{Q}}/\mathbb{Q})\) permutes \(\{u^2_\mathfrak{c}\}\) through the generalized dihedral action.* 

*Proof.* The boxed form exhibits \(u^2_\mathfrak{c}\) as \(\beta(\beta - 1728)\) times \(F(\Lambda_\mathfrak{c}; \mathbb{Z}[i])\) with \(F = -12\,g_2(\Lambda)/g_2(L)\), isobaric of joint weight \(0\). Apply \(\sigma\): the \(\beta\)-factor translates by the classical ring class reciprocity (the same input as moduli-invariants.md §5.5), and Lemma T gives \(\sigma F = F(s^{-1}\Lambda_\mathfrak{c}; s^{-1}\mathbb{Z}[i])\). Rescale jointly by a generator \(\nu\) of the principal ideal \((s^{-1}\mathbb{Z}[i])^{-1}\) (PID): \(\Lambda' := \nu s^{-1}\Lambda_\mathfrak{c}\) satisfies \(\Lambda' \subseteq \mathbb{Z}[i]\), \(\Lambda'\mathbb{Z}[i] = \nu s^{-1}\mathbb{Z}[i] = \mathbb{Z}[i]\) (primitive), index \(n\), and \([\Lambda'] = [s^{-1}\Lambda_\mathfrak{c}] = \mathfrak{c}(\sigma)^{-1}\mathfrak{c}\) (the idele class group modulo \(K^\times\hat{\mathcal{O}}_n^\times\mathbb{C}^\times\) *is* \(\mathrm{Cl}(\mathcal{O}_n)\), Artin-identified with \(\mathrm{Gal}(H_n/K)\)). So \(\sigma F = F(\Lambda'; \mathbb{Z}[i])\), and this is the \(u^2\)-datum of the class \(\mathfrak{c}(\sigma)^{-1}\mathfrak{c}\): the representative \(\Lambda'\) is determined up to \(\mu_4\), and \(F\) has weight \(4\) in \(\Lambda\) alone, so \(i^{-4} = 1\) kills the ambiguity — **at square level there is no cocycle at all**. \(\square\)

**Lemma D (archimedean dominance of the principal class).** For every \(n \ge 2\) and every class \(\mathfrak{c} \neq 1\): \(|u_\mathfrak{c}| < |u_1|\); explicitly \(|u_\mathfrak{c}|/|u_1| \le 10\,e^{-\pi n}\).

*Proof.* By §2, \(|u_\mathfrak{c}| = y\,|j'(\tau)|/(n\Omega)\) evaluated at the reduced CM point \(\tau_\mathfrak{c} = \tfrac{-B + 2ni}{2A}\), so \(|u_\mathfrak{c}| = |j'(\tau_\mathfrak{c})|/(A_\mathfrak{c}\Omega)\) with \(A_1 = 1\) and \(A_\mathfrak{c} \ge 2\) for \(\mathfrak{c} \ne 1\) (a reduced form of even discriminant with \(A = 1\) has \(B = 0\), hence is principal). On the fundamental domain \(|q| \le e^{-\pi\sqrt3} < \tfrac1{230}\), and the crude bounds \(\sigma_3(k) \le k^4\), \(\sigma_5(k) \le k^6\) give \(|E_4| \le 2.12\), \(|E_6| \le 3.9\), \(|\Delta_q| \ge 0.90\,|q|\), so \(|j'| = 2\pi|E_4|^2|E_6|/|\Delta_q| \le 2\pi \cdot 19.5\, e^{2\pi y}\); while at \(\tau = ni\) (\(q = e^{-2\pi n}\) positive, \(n \ge 2\)): \(E_4 \ge 1\), \(E_6 \ge 0.998\), \(\Delta_q \le q\), so \(|j'(ni)| \ge 2\pi\cdot 0.998\, e^{2\pi n}\). Hence
$$
\frac{|u_\mathfrak{c}|}{|u_1|} \;\le\; \frac{19.5}{0.998}\cdot\frac{1}{A}\;e^{2\pi n(\frac1A - 1)} \;\le\; 10\, e^{-\pi n} \;<\; \tfrac1{50} \quad (n \ge 2). \qquad\square
$$

> **Theorem 5 (irreducibility, every level).** For every \(n \ge 2\):
> 1. \(\mathrm{Gal}(\bar{\mathbb{Q}}/\mathbb{Q})\) permutes the multiset \(\{u^2_\mathfrak{c}\}\) generalized-dihedrally; hence \(\prod_\mathfrak{c}\bigl(y - (n^2v_\mathfrak{c})^2\bigr)\) is a **monic integer polynomial for all \(n\)** — the square-level rationality of §5.4, until now certified, is proved.
> 2. \(P^{(2)}_n\) **is irreducible over \(\mathbb{Q}\)** (any rational normalization of the roots), the square-phases of a level form a single Galois orbit, \([\mathbb{Q}(v^2_\mathfrak{c}) : \mathbb{Q}] = h(-4n^2)\) with \(v^2_\mathfrak{c} \in H_n\), and the Galois image is contained in (at the computed levels: equal to) the generalized dihedral group of \(\mathrm{Cl}(\mathcal{O}_n)\).
> 3. The same holds for \(P^{(6)}_n\).

*Proof.* (1) Stability under \(\mathrm{Gal}(\bar{\mathbb{Q}}/K)\) is the translation law; complex conjugation is the mirror law; any other \(\sigma\) is a composite. Symmetric functions are then rational, and the roots \((n^2v_\mathfrak{c})^2\) are algebraic integers (§5.3(iii)). (2) By Artin surjectivity the translations act transitively on \(\mathrm{Cl}(\mathcal{O}_n)\), so all \(u^2_\mathfrak{c}\) are conjugate and \(P^{(2)}_n = m^{|T|}\) with \(m\) irreducible and \(T = \{\mathfrak{t} : u^2_\mathfrak{t} = u^2_1\}\) a subgroup (the coincidence relation is translation-invariant). Any \(\mathfrak{t} \in T\) has \(|u_\mathfrak{t}| = |u_1|\), so Lemma D forces \(T = 1\). (3) Cube the translation law; \(|u^6_\mathfrak{t}| = |u^6_1|\) again forces \(T = 1\) by Lemma D. \(\blacksquare\)

The certified factorizations below are thus the verification of a theorem rather than the evidence for a conjecture — and the older conditional route (squarefreeness \(\Rightarrow\) irreducibility granting the translation law) is subsumed: both hypotheses are now proved. The full record (exact integer factorization over \(\mathbb{Q}\), all levels \(2 \le n \le 16\); `python3 scripts/mass_law_and_irreducibility.py irred`):
...
| \(P^{(2)}_n(y)\) | \(h\) | **yes, for every \(n \ge 2\) — Theorem 5**; verified exactly for \(2 \le n \le 16\), with squarefreeness certificates \(\gcd\bigl(P^{(2)}, P^{(2)\prime}\bigr) = 1\) |
| \(P^{(6)}_n(y)\) | \(h\) | **yes, for every \(n \ge 2\) — Theorem 5(3)**; verified exactly for \(2 \le n \le 16\): cubing the square-phases loses no degree, \(\mathbb{Q}\bigl((n^2v_\mathfrak{c})^6\bigr) = \mathbb{Q}\bigl((\lambda_n v_\mathfrak{c})^2\bigr)\) of degree \(h\) |
| \(P^{(2)}_n(x^2)\) (the first-power disk polynomial) | \(2h\) | **yes at every computed level** \(2 \le n \le 16\) (the first-power refinement of Theorem 5 remains open in general) |
...
Two remarks on the mechanism. *(i)* The contrast with the hyperbolic §5.10 is instructive: there, irreducibility for all levels still rests on two open hypotheses (the first-power descent and \(T = 1\)); here both evaporated — the descent because the square-phase has the root-free \(g_2\)-form, and \(T = 1\) because the coincidence set is a *subgroup*, so it only had to be tested against the archimedeanly dominant principal class. *(ii)* Lemma D is where the tangency geometry enters the Galois theory: the principal class is the row of disks tangent to \(\hat{\mathbb{R}}\), and its CM point \(ni\) escapes to the cusp while every other class stays at height \(\le n/2\) — the Ford row alone carries the archimedean growth, and that alone pins the factorization type of \(P^{(2)}_n\) for every \(n\).
...
**6.2 The first-power descent.** The square level is now closed: \(\gamma_2(w)R_4(\Lambda) = 12\,g_2(\Lambda)/g_2(\mathbb{Z}[i])\) is root-free and Theorem 5 (§5.6) proves rationality, integrality and irreducibility of \(P^{(2)}_n\) for every \(n\). What remains is one power lower: does the *first-power* disk polynomial \(P^{(2)}_n(x^2)\) stay irreducible for all \(n\) (certified for \(n \le 16\))? Equivalently: is \(v_\mathfrak{c}\) never a square-root that falls into \(\mathbb{Q}(v_\mathfrak{c}^2)\)? The natural attack is a weight-2 refinement of Lemma T — transporting \(g\)-expressions of odd \(\mu_4\)-weight, where the unit ambiguity survives as a genuine \(\pm\)-cocycle tied to the orientation data of §5.2 — the exact Euclidean shadow of the hyperbolic first-power problem.
...
**6.7 The translation law: done at squares, open at first powers.** The square-level law \(\sigma(u^2_\mathfrak{c}) = u^2_{\mathfrak{c}(\sigma)^{-1}\mathfrak{c}}\) is now proved (§5.6: Lemma T plus the \(g_2\)-closed form; no cocycle survives at weight 4), delivering the generalized dihedral containment and Theorem 5 — as anticipated, strictly easier than the hyperbolic §5.5–5.8 chain. The remaining refinement is the first-power law for \(v_\mathfrak{c}\) itself (the \(\pm\)-cocycle of 6.2), and the determination of the exact Galois image for all \(n\) (equality with the generalized dihedral group — equivalent to \(v^2_\mathfrak{c}\) generating \(H_n \cap \mathbb{R}\)-type subfields sharply; certified at the computed levels).
...
- [scripts/mass_law_and_irreducibility.py](scripts/mass_law_and_irreducibility.py) — proof verification for Theorem 4 (`... mass`: M1 at random \(\tau\); the exact recursion vs. the closed form, \(n \le 60\); the symbolic generating-function identities; the class-by-class sign analysis, \(n \le 20\)), the irreducibility record of §5.6 (`... irred`: \(H\), \(P^{(2)}\), \(P^{(6)}\), \(P^{(2)}(x^2)\) factored exactly with squarefreeness certificates, \(2 \le n \le 16\)), and the computational ingredients of Theorem 5 (`... theorem5`: the root-free \(g_2\)-closed form at every class including \(3 \mid n\), the \(E_4(2i)/E_4(i) = \tfrac{11}{16}\) anchor, and Lemma D's dominance ratios against the \(10e^{-\pi n}\) bound).
