...
So in every certified case the multiset \(\{u_f\}\) at level \(n\) is the root set of a **single reciprocal polynomial with rational coefficients**, of the minimal degree \(h\) — e.g. at \(n = 9, 11, 13\) all four \(u\)'s are roots of \((x^2 - S_Ax + 1)(x^2 - S_Bx + 1) \in \mathbb{Q}[x]\). That the \(u_f\) of a level are the roots of one polynomial with **integer** coefficients is a theorem, not an observation: see §5.9, where it is deduced unconditionally from Theorem A (in degree \(12h\)) and the degree-\(h\) form above is certified up to \(n = 17\). Moreover neither polynomial factors: the \(u_f\) of a level form a *single Galois orbit* and the degree-\(h\) polynomial is the common minimal polynomial of the phase units — §5.10.
...
### 5.10 Irreducibility: one level, one Galois orbit

The polynomials of §5.9 are as small as they can be, and they do not factor: **at every computed level \(Q_n\) and \(\Psi_n\) are irreducible over \(\mathbb{Q}\)**. Equivalently, up to its leading constant \(Q_n\) is the *minimal polynomial of every phase unit of its level*, the \(h\) units \(u_f\) form a single \(\mathrm{Gal}(\bar{\mathbb{Q}}/\mathbb{Q})\)-orbit, and nothing of degree \(< h\) kills any of them. The mechanism is one half of Theorem A — the Artin map realizes *every* translation of \(\mathrm{Cl}(\mathcal{O}_D)\), and translations act transitively on the classes — and it reduces irreducibility to a single finite exact computation per level.

> **Theorem D.** Fix an odd \(n \ge 3\), let \(u_f\), \(\Psi_n\), \(\Phi_n\), \(Q_n\) be as in §5.9, and grant Theorem A (§5.8). Then:
> 1. All twelfth powers \(u_f^{12}\) (\(f \in \mathrm{Cl}(\mathcal{O}_D)\)) are conjugate over \(\mathbb{Q}\); the coincidence set \(T := \{t \in \mathrm{Cl}(\mathcal{O}_D) : u_t^{12} = u_1^{12}\}\) is a **subgroup**, and
> $$
> \Psi_n \;=\; m^{\,|T|} \qquad\text{for a single monic irreducible } m \in \mathbb{Q}[y] \text{ of degree } [\mathrm{Cl}:T].
> $$
> In particular \(\Psi_n\) is either irreducible or a perfect power, and: \(\;\Psi_n\) **is irreducible \(\iff\) the \(u_f^{12}\) are pairwise distinct \(\iff\) \(\Psi_n\) is squarefree** — and squarefreeness of the certified \(\Psi_n\) is a finite exact gcd computation.
> 2. If moreover \(\Phi_n \in \mathbb{Q}[x]\) (the first-power descent of §5.8, certified at every computed level) and \(T = 1\), then \(\Phi_n\) is the minimal polynomial of each \(u_f\): **\(Q_n\) is irreducible over \(\mathbb{Q}\)**, the \(u_f\) are pairwise distinct and form one Galois orbit, \([\mathbb{Q}(u_f) : \mathbb{Q}] = h\), and \(\mathbb{Q}(u_f) = \mathbb{Q}(u_f^{12})\).

*Proof.* (1) The Artin map \(\mathrm{Gal}(\bar{\mathbb{Q}}/K) \twoheadrightarrow \mathrm{Gal}(H/K) \cong \mathrm{Cl}(\mathcal{O}_D)\) is onto (class field theory for the ring class field), so for every class \(c\) there is a \(\sigma_c\) fixing \(K\) with \(e(\sigma_c) = +1\), \(\mathfrak{c}(\sigma_c) = c\), and Theorem A reads \(\sigma_c(u_f^{12}) = u_{fc}^{12}\). Taking \(c = f^{-1}g\) exhibits every \(u_g^{12}\) as a conjugate of every \(u_f^{12}\). Applying \(\sigma_c\) to an equality \(u_f^{12} = u_g^{12}\) shows the relation \(u_f^{12} = u_g^{12}\) is translation-invariant; hence it holds iff \(f^{-1}g \in T\), \(T\) is a subgroup (translate \(u_t^{12} = u_1^{12}\) by \(t'\in T\)), and the fibers of \(f \mapsto u_f^{12}\) are exactly the cosets of \(T\) — every value is taken with multiplicity \(|T|\). Let \(m\) be the minimal polynomial of \(u_1^{12}\) over \(\mathbb{Q}\). Every root of \(\Psi_n\) is a conjugate of \(u_1^{12}\), hence a root of \(m\); conversely every root of \(m\) is some \(\sigma(u_1^{12}) = u_{1^{e(\sigma)}\mathfrak{c}(\sigma)}^{12} = u_{\mathfrak{c}(\sigma)}^{12}\), a root of \(\Psi_n\). So the distinct roots of \(\Psi_n\) are precisely the roots of the separable \(m\), each with multiplicity \(|T|\): \(\Psi_n = m^{|T|}\), \(\deg m = [\mathrm{Cl}:T]\). The stated equivalences follow.

(2) \(T = 1\) gives \(\deg m = h\), so \([\mathbb{Q}(u_f^{12}) : \mathbb{Q}] = h\) for every \(f\), and \([\mathbb{Q}(u_f) : \mathbb{Q}] \ge h\) since \(\mathbb{Q}(u_f) \supseteq \mathbb{Q}(u_f^{12})\). The monic \(\Phi_n \in \mathbb{Q}[x]\) has degree \(h\) and kills \(u_f\), so it is the minimal polynomial of \(u_f\); minimal polynomials are irreducible, and equality of degrees forces \(\mathbb{Q}(u_f) = \mathbb{Q}(u_f^{12})\). Distinctness of the \(u_f\) and the single orbit are immediate (a minimal polynomial is separable and its roots are one orbit). \(\square\)

**Remarks.**
- **\(T\) avoids \(\mathfrak{r}_n\).** \(\mathfrak{r}_n \in T\) would say \(u_1^{-12} = u_{\mathfrak{r}_n}^{12} = u_1^{12}\), i.e. \(|u_1| = 1\), contradicting the certified growth \(|u_1| = 4.6\cdot10^3, 6.8\cdot10^6, \dots\) (§5.9) for \(n \ge 5\). So for \(h \le 2\) (\(n = 5, 7\); \(n=3\) is trivial), \(T = 1\) needs no further input. At \(n = 9\) (\(\mathrm{Cl} = \mathbb{Z}/4\)) the only subgroup avoiding \(\mathfrak{r}\) is \(1\), so \(T = 1\) there too; at the \((\mathbb{Z}/2)^2\)- and \(\mathbb{Z}/4\times\mathbb{Z}/2\)-levels \(T = 1\) is certified by the pairwise-distinct twelfth powers (equivalently, by exact squarefreeness of \(\Psi_n\); at \(n = 11, 13, 17\) it also follows from the certified pair-sums — \(u_t^{12} = u_1^{12}\) with all classes ambiguous would force \(u_t = \pm u_1\), i.e. \(S_B = \pm S_A\), contradicting \(S_A \pm S_B \ne 0\) of §5.6/§5.8).
- **Sharpness, and the contrast with \(P_n\).** Once \(\Phi_n \in \mathbb{Q}[x]\) and \(T = 1\), no polynomial of degree \(< h\) vanishes at any \(u_f\), so the degree-\(h\) form is optimal; the unconditional degree-\(12h\) polynomial \(P_n\), by contrast, is *never* irreducible (\(\Phi_n\) divides \(d_n^{-1}P_n\) properly; already \(P_3 = x^{12} - 1\)).
- **Pair-sum polynomials are minimal too.** \(S = u + u^{-1}\) generates the index-2 subfield \(\mathbb{Q}(S) \subset \mathbb{Q}(u)\), so \([\mathbb{Q}(S):\mathbb{Q}] = h/2\): the certified quadratic conjugacies of §5.6 (\(n = 9, 11, 13, 17\)) and the rational quartic of the \(n = 15\) pair-sums (§5.8) are exactly the minimal polynomials of the pair-sums — and \(u_f\) is a norm-one element of the quadratic extension \(\mathbb{Q}(u_f)/\mathbb{Q}(S)\) (its conjugate there is \(1/u_f\)).
- **What remains for general \(n\).** Only \(T = 1\) (never violated; a violation would make \(\Psi_n\) a perfect power) and the first-power descent already listed among the open questions. Given both, Theorem D settles every level at once.

**Certified verification** (`python3 scripts/uf_irreducibility.py`, all odd \(3 \le n \le 17\)): the script rebuilds the certified \(Q_n, \Psi_n\) of §5.9 and proves irreducibility twice, independently.

*Route 1 — Theorem D.* Exact squarefreeness of \(\Psi_n\) (Fraction-arithmetic \(\gcd(\Psi_n, \Psi_n')\)); with Theorem A this makes \(\Psi_n\) irreducible, and the certified rationality of \(\Phi_n\) then makes \(Q_n\) irreducible.

*Route 2 — polynomial facts, no Theorem A.* For \(h \le 4\), a complete integer-arithmetic decision: the polynomials are palindromic, so any rational root besides \(\pm1\) (excluded by exact evaluation) comes as a pair \(\{t, 1/t\}\) giving a 2+2 split, and every rational 2+2 split of the monicized quartic forces an *integer* root of its resolvent cubic; the script enumerates these exactly (monotone-piece bisection in pure integer arithmetic) and refutes each by perfect-square and exact-division tests. Where the Galois group permits, a one-prime certificate (irreducible mod \(p\), \(p \nmid \mathrm{lc}\)); and for every level a **root-subset integrality certificate**: \(\mathrm{lc}\cdot u_f\) is an algebraic integer, so a monic rational factor of \(\Phi_n\) would have all its \(e_i(\mathrm{lc}\cdot u_f)\) in \(\mathbb{Z}\) over its root subset — every proper nonempty subset (all \(2^h - 2\), e.g. \(254\) at \(n = 15\)) is excluded by a margin at least \(10^{100}\) times the error bound. A sympy exact factorization cross-checks each verdict.

| \(n\) | \(h\) | \(\Psi_n\) squarefree (exact) | degree-\(\le4\) exact decision | one-prime certificate \(Q_n\) / \(\Psi_n\) | subsets excluded \(Q_n\); \(\Psi_n\) | \(\mathrm{Gal}(Q_n)\) |
|---|---|---|---|---|---|---|
| 3 | 1 | ✓ | ✓ (linear) | \(p=2\) / \(2\) | — | \(1\) |
| 5 | 2 | ✓ | ✓ (disc \(\ne\square\)) | \(11\) / \(13\) | 2/2; 2/2 | \(\mathbb{Z}/2\) |
| 7 | 2 | ✓ | ✓ (disc \(\ne\square\)) | \(5\) / \(17\) | 2/2; 2/2 | \(\mathbb{Z}/2\) |
| 9 | 4 | ✓ | ✓ (resolvent) | \(3\) / \(3\) | 14/14; 14/14 | \(D_4\) |
| 11 | 4 | ✓ | ✓ (resolvent) | none exists | 14/14; 14/14 | \(V_4\) |
| 13 | 4 | ✓ | ✓ (resolvent) | none exists | 14/14; 14/14 | \(V_4\) |
| 15 | 8 | ✓ | — | none exists | 254/254; 254/254 | order-16 dihedral image (text) |
| 17 | 4 | ✓ | ✓ (resolvent) | none exists | 14/14; 14/14 | \(V_4\) |

**The Galois groups come out exactly as Theorem A predicts.** The predicted image of \(\mathrm{Gal}(\bar{\mathbb{Q}}/\mathbb{Q})\) in \(\mathrm{Sym}\{u_f\}\) is the generalized dihedral \(\mathrm{Cl} \rtimes \langle\mathrm{inv}\rangle\) acting on \(\mathrm{Cl}(\mathcal{O}_D)\), and inversion is trivial on a 2-torsion class group — so at \(n = 11, 13, 17\) the image is just the **regular Klein four-group**. A \(V_4\)-quartic contains no 4-cycle, hence — like \(x^4+1\) — is *reducible modulo every prime* while irreducible over \(\mathbb{Q}\); the script observes exactly that (only patterns \([1,1,1,1]\) and \([2,2]\) over all good primes \(< 600\)), and the classical criterion (irreducible quartic whose resolvent cubic splits completely over \(\mathbb{Q}\) — here: three integer roots, each refuted as an actual split by the exact perfect-square and division tests) pins \(\mathrm{Gal}(Q_n) \cong V_4\) *unconditionally* at \(n = 11, 13, 17\). At \(n = 9\) (\(\mathrm{Cl} = \mathbb{Z}/4\)) the image is \(D_4\): a one-prime certificate exists (\(p = 3\)), and the observed pattern \([1,1,2]\) (an odd involution with fixed points, impossible in \(C_4\)) pins \(\mathrm{Gal}(Q_9) \cong D_4\) unconditionally. At \(n = 15\) the image has order 16 with cycle types \([1^8], [1^42^2], [2^4], [4,4]\) and no 8-cycle — again no one-prime certificate can exist — and the observed Frobenius patterns are precisely those four types. At every level the observed patterns lie inside the predicted cycle types, and the parity check matches exactly: \(\operatorname{disc} Q_n\) is a perfect square iff the predicted image is even (\(n = 3, 11, 13, 15, 17\) square; \(n = 5, 7, 9\) not) — an independent first-power-level fingerprint of the §5.8 Galois action, visible in exact integer arithmetic.
...
- A general-\(n\) proof that the twelfth powers \(u_f^{12}\) are pairwise distinct (\(T = 1\) in Theorem D of §5.10) — the only hypothesis besides the first-power descent separating the per-level irreducibility proofs from an all-\(n\) theorem.
...
- Irreducibility of \(Q_n, \Psi_n\) (§5.10): [scripts/uf_irreducibility.py](scripts/uf_irreducibility.py) — `python3 scripts/uf_irreducibility.py` (both proof routes, all odd \(n \le 17\)); `--psi-subsets` adds the root-subset certificate for \(\Psi_n\) at elevated precision (up to \(3368\) digits at \(n = 15\)).
